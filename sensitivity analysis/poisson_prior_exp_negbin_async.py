...
class negbin_gamma():
    def __init__(self, shape_a, rate_a, shape_b, rate_b, latent=11, rat=0.9):
        self.shape_a = float(shape_a)
        self.shape_b = float(shape_b)
        self.scale_a = 1.0 / rate_a
        self.scale_b = 1.0 / rate_b
    ...
    def sample(self, shape=(1, 1, 100)):
        latent_a = np.random.gamma(self.shape_a, self.scale_a, (self.latent, shape[0], shape[-1]))
        latent_b = np.random.gamma(self.shape_b, self.scale_b, (self.latent, shape[1], shape[-1]))
        rate = np.einsum('kj...,ki...', latent_a, latent_b).T
        self.rmat = np.random.negative_binomial(self.bin_rate,1-rate/(self.bin_rate+rate))
        return self.rmat
...
def run_exp_negbin(exp, k_latent=25, n_repeats=5, shape=(1000, 1000), sampling_percentage=0.15, p_obs=0.5):
    res = []
    for x in range(n_repeats):
        model = negbin_gamma(exp[0], exp[1], exp[2], exp[3], k_latent, p_obs)
        model.sample((shape[0], shape[1], 1))
        res_d = {"p_a": exp[0], "p_b": exp[1], "p_c": exp[2], "p_d": exp[3], "t_latent_dim": k_latent, "p_obs": p_obs}
        res_d.update(model.theoretical())
        res_d.update(empirical(model.rmat[:, :, 0], None, ns=int(sampling_percentage * shape[0] * shape[1])))
...
p_obs = [1.0, 10.0, 100.0]
rand_n = np.random.randint(1000)

#### Experiment v2 4####
N_EXP = 20  # number of experiments per hyperparameter setting
NS = 0.20
N_COLS = 500
N_ROWS = 500

k_latent_vals = [25,50,75,100,125,150]
p_obs = [50.0, 100.0, 500.0, 1000.0, 5000.0]
rand_n = np.random.randint(1000)
...
def experiment_worker(k_latent_vals, p_obs, hyperparameters, N_EXP, N_COLS, N_ROWS, NS, exp_n):
    ...
    for k_latent in k_latent_vals:
        for obs in p_obs:
            t2 = time.time()
            temp = run_exp_negbin(hyperparameters, k_latent=k_latent, n_repeats=N_EXP, shape=(N_COLS, N_ROWS),
    ...
    df_temp.to_csv(
        "exp_poisson_gamma_negbin_e" + str(rand_n) + "_" + str(int(NS * 100)) + "_" + str(N_EXP) + "_cols_" + str(N_COLS) + "_rows_" + str(
...
df.to_csv(
    "final_exp_poisson_gamma_negbin_e" + str(rand_n) + "_" + str(int(NS * 100)) + "_" + str(N_EXP) + "_cols_" + str(N_COLS) + "_rows_" + str(
        N_ROWS) + "_c" + ".csv")
